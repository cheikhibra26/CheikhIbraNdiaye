import streamlit as cv
# configuration de la page
cv .set_page_config(page_title="cv - CHEIKH IBRA NDIAYE", page_icon="photo.pjg",layout= "centered")#
# titre
cv.title("CHEIKH IBRA NDIAYE")

cv. write ("Ingénieur en  aménagegement et gestion urbaine en Afrique doté d'une compétence de planification du territoire,aménagement et diagnostique du territoire .En plus,je suis en téchnicien supérieur en géomatique avec la maitrise des outils d'information géographie (SIG),cartographie,télédection,Pilote drone,Topographie. Je suis certifié sur la  collect de données quantitative et qualitative au niveau de l'ANSD. A-cela, s'ajoute mes compétences en labaratoir géotechnique  ")

cv.title("PARCOURS")

cv.subheader("Master en Aménagement et Gestion Urbaine en Afrique")
cv. write ("Université  Cheikh Anta Diop de Dakar ")
cv.write ("Année:2026")
cv.subheader("Licence en Gouvernance Térritoriale et Développement Local Option:Aménagement du Territoire et Développement Local ")
cv. write ("Université  Cheikh Anta Diop de Dakar:Institut de la Gouvernance Territorial et du  Développement Local ")
cv.write ("Année 2024")
cv.subheader("Brévet Téchnique Supérieure en  Géomatique  ")
cv. write ("Centrte D'entrepreneuriat et de Développement Téchnique CEDT Le G15 ")
cv.write ("Année 2027")
cv.subheader("Baccalauriat en Science Humaine et Social ")
cv. write ("Université Cheikh Anta Diop de Dakar ")
cv.write ("Année:2020")

cv.title ("CERTIFICATION")

cv.subheader(" Attestation en Laboratoir Géotechnique ")
cv. write("Centre de Formation Professionnel des Travaux Public ")

cv.subheader("Attestation en Téchnologie Immersive VR/AR/MR ")
cv. write ("CURI")

cv.subheader("Attestation en Infographie")
cv.write ("Skill Up")
cv. subheader (" Attestation en Prestation de Service Rnu")
cv.write ("ANSD ")
cv. subheader(" Attestation en Informatique et Internet ")
cv.write ("FORCE-N")
cv. subheader ("Attestation en Intélligence Artificielle  Pour Tous")
cv.write (" FORCE-N ")
cv. subheader (" Attestation en Traitement de Donnée")
cv.write (" FORCE-N")
cv.subheader (" Attéstation du Projet de Formation sur la Valorisation des Ressources et Innovation Exploitation Economique du Sénégal ") 
cv.write ("UNCHK")

with cv.sidebar:
   
    
    cv.title("INGENIEUR AMENAGISTE /TECHNICIEN  GEOMATIQUE")  
    
    cv.write("Email:wwwcheikhibrandiaye@gmail.com")
    
    cv.write("Adresse:Dakar/Sénégal")
    
    cv.write("Situation Matrimonial:Cclibataire sans enfant")
   
    cv.title ("COMPETENCES")
   
    cv.markdown("""
    - Microsofte office
            
    
    - Cartographie 
    
    - Topographie 
    
    - Planification et Diagnostique Territoriale 
    
    - Déssin Plan Architecture
          
    - Collécte données 
    
    - Creation d'une Base de donnée
    
    - Aménagement du Territoire
    
    - Diagnostique Spatiale
     -Projection spatiale
    
    - Essaie en  Laboratoire Géotechnique
    
    - Infographie
    
    - Numérisation 
    
    - Programmation
    
    - Télédectection
    """)
    
    
